 ...
 */
 static struct clk	*otg_clock = NULL;		//janged 아래에 있던거 가져욤 
...
//	int i;
	...
	s3c_gpio_setpull(S3C64XX_GPK(1), S3C_GPIO_PULL_NONE);
...
static int udc_enable(struct s3c_udc *dev)
{
	...
	s3c_gpio_cfgpin(S3C64XX_GPM(3), S3C_GPIO_SFN(0));

	//GPM3 PULL UP Down Disable
...
static void reconfig_usbd(void)
{
	...
	uTemp = writel(CORE_SOFT_RESET, S3C_UDC_OTG_GRSTCTL);
	//janged modify for samsung error
	#if 0
	writel(	0<<15		/* PHY Low Power Clock sel*/
		...
		S3C_UDC_OTG_GUSBCFG);
	#else
	writel(	1<<15		/* PHY Low Power Clock sel*/
		|1<<14		/* Non-Periodic TxFIFO Rewind Enable*/
		|0x5<<10	/* Turnaround time*/
		|0<<9|0<<8	/* [0:HNP disable, 1:HNP enable][ 0:SRP disable, 1:SRP enable] H1= 1,1*/
		|0<<7		/* Ulpi DDR sel*/
 ...
 */
//static struct clk	*otg_clock = NULL; janged 위치 이동 
