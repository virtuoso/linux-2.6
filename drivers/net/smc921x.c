...
#define SMSC_CHIPNAME		"smsc921x"
#define SMSC_DRV_VERSION	"2009-05-16"

MODULE_LICENSE("GPL");

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,23)
# define NAPI_SUPPORTED
#endif
...
{
	/* ToDo: bushi:
	 *     need to workaround for read-after-write restriction
	 */
...
{
	writesl((void __iomem *)(pdata->ioaddr + TX_DATA_FIFO), buf, wordcount);
}
...
smsc911x_rx_readfifo(struct smsc911x_data *pdata, unsigned int *buf,
		     unsigned int wordcount)
{
	readsl((void __iomem *)(pdata->ioaddr + RX_DATA_FIFO), buf, wordcount);
}
...
static inline u32 smsc911x_reg_read(struct smsc911x_data *pdata, u32 reg)
{
	...
	/* ToDo: bushi:
	 *     need to workaround for read-after-read restriction
	 */
...
static int smsc911x_phy_initialise_external(struct smsc911x_data *pdata)
{
	...
	unsigned int hwcfg;
	unsigned int phyid1 = 0xFFFF;
	unsigned int phyid2 = 0xFFFF;
	...
	if (hwcfg & HW_CFG_EXT_PHY_DET_) {
		...
		for (address = 31; address > 0; address--) {
			unsigned int _phyid1, _phyid2;
			pdata->mii.phy_id = address;
			_phyid1 = smsc911x_phy_read(pdata, MII_PHYSID1);
			_phyid2 = smsc911x_phy_read(pdata, MII_PHYSID2);
			if ((_phyid1 != 0xFFFFU) || (_phyid2 != 0xFFFFU)) {
				phyid1 = _phyid1;
				phyid2 = _phyid2;
				target_address = address;
				SMSC_TRACE("Detected PHY at address#"
						   "%u, 0x%04X:0x%04X",
						   address, phyid1, phyid2);
				/* RTL830{5|6} specific */
				if ((_phyid1 == 0x001C)
					    && ((_phyid2 & 0xfff0) == 0xC850)) {
					unsigned int temp;
					/* make sure H/W pin configurations */
					temp = smsc911x_phy_read(pdata, 24);
					SMSC_TRACE("RTL:MII24: 0x%08x\n", temp);
#if 0
					/* force to auto-crossover correct */
					temp |= (1<<7);
					smsc911x_phy_write(pdata, 24, temp);
#endif
				}
			}
		}
		...
		if ((phyid1 == 0xFFFFU) && (phyid2 == 0xFFFFU)) {
			...
			pdata->mii.phy_id = target_address;
...
#if 0
		pdata->link_poll_timer.expires = round_jiffies(jiffies + 2*HZ);
		add_timer(&pdata->link_poll_timer);
#else
		mod_timer(&pdata->link_poll_timer, round_jiffies(jiffies + 2*HZ));
#endif
	} else {
...
static int __devinit smsc911x_drv_probe(struct platform_device *pdev)
{
	...
	printk("%s, %d, start= %x, ioaddr = %x \n", __FUNCTION__, __LINE__, res->start, res_size);

	/* copy config parameters across if present, otherwise pdata
	 * defaults to zeros */
	if (pdev->dev.platform_data) {
		struct smsc921x_platform_config *config =
