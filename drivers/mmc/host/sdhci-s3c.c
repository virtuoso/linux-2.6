...
static void sdhci_s3c_check_sclk(struct sdhci_host *host)
{
	...
	if (get_curclk(tmp) != ourhost->cur_clk) {
		...
		printk("%d, %d ++++++++++++++++++\n", get_curclk(tmp), ourhost->cur_clk);
...
//		return sdhci_s3c_get_max_clk(host) / 500000;
#endif
}
...
//janged
unsigned int mmc_clock_for_woong = 0;
unsigned int mmc_clock_for_mmc0 = 0;
EXPORT_SYMBOL(mmc_clock_for_woong);
EXPORT_SYMBOL(mmc_clock_for_mmc0);
void sdhci_s3c_change_clock(struct sdhci_host *host, unsigned int clock)
{
	struct sdhci_s3c *ourhost = to_s3c(host);
...
//	int src;
	...
	#if 0		//janged_mmc modify for mmc 신뢰성 
...
//		 best_src, clock, best);
	...
	#else
	{
		struct clk *clk = ourhost->clk_bus[2];
		ourhost->cur_clk = 2;
		host->max_clk = clk_get_rate(clk);
		host->timeout_clk = host->max_clk / 1000;
		...
		ctrl = readl(host->ioaddr + S3C_SDHCI_CONTROL2);
		ctrl &= ~S3C_SDHCI_CTRL2_SELBASECLK_MASK;
		ctrl |= 2 << S3C_SDHCI_CTRL2_SELBASECLK_SHIFT;
		writel(ctrl, host->ioaddr + S3C_SDHCI_CONTROL2);
	}
	#endif
	...
	{
		mmc_clock_for_mmc0 = clock;
	}

	if(!strcmp(mmc_hostname(host->mmc), "mmc1"))
	{
		mmc_clock_for_woong = clock;
	}
...
static int __devinit sdhci_s3c_probe(struct platform_device *pdev)
{
	...
	sc->clk_io = clk_get(dev, "mmc_bus");		//janged
	//sc->clk_io = clk_get(dev, "mout_epll");
	if (IS_ERR(sc->clk_io)) {
	...
	//janged_mmc modify for mmc 신뢰성  이한줄이 그동안 사람을....
	//6410 Data Sheet 7c200080 의 4,5번 비트 1로 해야 함.
	//이건 s3c6400_clock.c에서 clk_src 레지스터 설정과 동기화 되어야 함
	//sc->cur_clk = 2;
...
#ifdef CONFIG_CPU_S3C6410		//janged
//	if(!strcmp(mmc_hostname(host->mmc), "mmc1"))		//janged_clk100119
	{
...
#ifdef CONFIG_PM
extern void s3c6410_setup_sdhci0_cfg_gpio_for_sleep(void);
extern void s3c6410_setup_sdhci1_cfg_gpio_for_sleep(void);
extern void setup_sdhci0_irq_cd_sleep(void);

static int sdhci_s3c_suspend(struct platform_device *dev, pm_message_t pm)
{
        struct sdhci_host *host = platform_get_drvdata(dev);

		//printk("sdhci_s3c_suspend\n");        
