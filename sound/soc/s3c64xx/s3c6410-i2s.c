...
#include <plat/regs-clock.h>	//janged_clk

#include <linux/spinlock.h>		//janged_clk
...
DEFINE_MUTEX(janged_clk);	//jagned_clk for i2s <-> uart
...
extern spinlock_t eplllock; 	/* Initialize */ //janged_clk
...
*/
		...
		case 96000:
			//janged º¯°æ 
			#if 1
			if((s3c6410_i2s.clk_rate != 49152000) || (clk_get_rate(clk) != 49152000))
			{
				u32 tmp;
				audio_enable_for_pm();
				lock_kernel();
				...
				tmp &= ~0x00000fff;
				writel(tmp | 0x222, S3C_CLK_DIV1);

				/* init uart_clock divider */
				tmp = __raw_readl(S3C_CLK_DIV2);
				tmp &= ~0x000fff00;
				writel(tmp | 0x00000, S3C_CLK_DIV2);

//				reconfig_movinand_for_i2s();
//				reconfig_sdcard_for_i2s();


				clk_enable(clk);
				unlock_kernel();
				s3c6410_i2s.clk_rate = clk_get_rate(s3c6410_i2s.audio_bus);
				printk("Setting FOUTepll to %dHz 1 +++++++++\n", s3c6410_i2s.clk_rate);
				clk_put(clk);
		...
		case 88200:
			#if 1
			if((s3c6410_i2s.clk_rate != 67738000) || (clk_get_rate(clk) != 67738000))
			{
				u32 tmp;
				audio_enable_for_pm();
				lock_kernel();
				...
				s3c6410_i2s.clk_rate = clk_get_rate(s3c6410_i2s.audio_bus);
				printk("Setting FOUTepll to %dHz 2 +++++++++\n", s3c6410_i2s.clk_rate);
				clk_put(clk);
				spin_unlock(&eplllock);
				unlock_kernel();
			}
			     ...
			     struct snd_soc_dai *dai)
{
	...
	//writel(0x00000c00, s3c6410_i2s.regs + S3C64XX_IISMOD);
...
struct snd_soc_dai s3c6410_i2s_v40_dai = {
	...
	.playback = {
		...
		.formats = SNDRV_PCM_FMTBIT_S8 | SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE, 
 		//SNDRV_PCM_FMTBIT_S8 | SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE, janged
	},
	.capture = {
		.channels_min = 2,		//janged
		.channels_max = 2,
		//janged modify
