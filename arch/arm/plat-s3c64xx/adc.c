 ...
 *        controller
 *
 * 2009-06-25: Modified by woong in order to support battery and board version
 * 
 */
...
//#define BIT12_AD_CONVERSION
#define BUG_TRACE
static int g_prescaler = 49;

static struct clk	*adc_clock;

static void __iomem 	*base_addr;
static int adc_port =  7;
...
unsigned int s3c_adc_convert(void)
{
    ...
    writel(0x0, base_addr + S3C_ADCCON);
    writel(S3C_ADCCON_PRSCEN|S3C_ADCCON_PRSCVL(g_prescaler)|S3C_ADCCON_SELMUX(adc_port)| S3C2410_ADCCON_ENABLE_START, base_addr + S3C_ADCCON);
	
#ifdef BUG_TRACE
	writel(readl(base_addr + S3C_ADCCON) | S3C_ADCCON_SELMUX(adc_port), base_addr + S3C_ADCCON);
#else
	writel(readl(base_addr + S3C_ADCCON) | S3C_ADCCON_SELMUX(adc_port) | S3C_ADCCON_STDBM, base_addr + S3C_ADCCON);
#endif
	udelay(10);
...
int read_adc_ch(int ch)
{
	static int prev_adc_port = 0;
	unsigned int retADCVal = 0;
	...
	prev_adc_port = adc_port = ch;
	
    mutex_unlock(&adc_mutex);
	retADCVal = s3c_adc_convert();

	return retADCVal;
}
EXPORT_SYMBOL(read_adc_ch);
...
static int s3c_adc_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	//woong
	//printk(KERN_INFO "\ns3c_adc_suspend() adccon 0x%x!\n", adccon);
	writel(0xcc74, base_addr + S3C_ADCCON);
	adccon = readl(base_addr + S3C_ADCCON);
	//printk(KERN_INFO "\ns3c_adc_suspend() adccon 0x%x!\n", adccon);
	//printk(KERN_INFO "\ns3c_adc_suspend() adcdly 0x%x!\n", adcdly);
...
{
	//printk(KERN_INFO "s3c_adc_resume() !\n");
	clk_enable(adc_clock);
