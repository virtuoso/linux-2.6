...
static struct platform_device *smdk6410_devices[] __initdata = {
	...
	&s3c_device_rtc,
	&s3c_device_i2c0,
	//&s3c_device_i2c1,    //woong  //이놈은 이상함...
	&s3c_device_spi0,
	&s3c_device_spi1,
	&s3c_device_keypad,
	&elice_device_powerkey, //Added by woong
	//&s3c_device_ts,		janged
	#ifdef CONFIG_SMC921X
	&s3c_device_smc911x,
	#endif
	//&s3c_device_lcd,	woong
//	&s3c_device_vpp,	woong
//	&s3c_device_mfc,	woong
	//&s3c_device_tvenc,	janged
	//&s3c_device_tvscaler,	janged
	//woong
#if 0	
	&s3c_device_rotator,
	&s3c_device_jpeg,
#endif
	//&s3c_device_nand,	janged
	//&s3c_device_onenand,	janged
	&s3c_device_usb,
	&s3c_device_usbgadget,
	//&s3c_device_usb_otghcd,	janged
#if 0 //woong	
	&s3c_device_fimc0,
	&s3c_device_fimc1,
	&s3c_device_g2d,
...
static void __init smdk6410_smc911x_set(void)
{
	...
	#else
	/* MAC/PHY h/w reset */
//	s3c_gpio_cfgpin(S3C_GPL11, S3C_GPL11_OUTP);
//	s3c_gpio_cfgpin(S3C64XX_GPL(11), 1);
	#if 0
...
{
	//s3c_device_nand.dev.platform_data = &s3c_nand_mtd_part_info;	janged
	//s3c_device_onenand.dev.platform_data = &s3c_onenand_data;		janged
...
void otg_phy_off(void) {
	...
	//janged 아래 값 대로 안하면 특정 세트들 USB 케이블 연결하자 마자 PC에 장치 인식 실패 
	//팝업 생성됨 
	writel(0x1, S3C_USBOTG_RSTCON);	
    writel(0, S3C_USBOTG_PHYCLK);
	writel(readl(S3C_USBOTG_PHYPWR)|(0x19<<1), S3C_USBOTG_PHYPWR);
	writel(readl(S3C_OTHERS)&~S3C_OTHERS_USB_SIG_MASK, S3C_OTHERS);
}
