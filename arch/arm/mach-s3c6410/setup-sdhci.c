...
char *s3c6410_hsmmc_clksrcs[4] = {
	...
	#else
	 [0] = "hsmmc",		//janged 아래 3개의 사항 절대 변경 하면 안됨 
	 [1] = "hsmmc", 
	 [2] = "mmc_bus",  //janged
	#endif
};
...
{
	//janged elisa는 데모 보드와 비교 하면 channel 0와 1이 반대임 
...
#if 1 /* only ch1 will have card detection pin. by scsuh. */		//janged
	s3c_gpio_cfgpin(S3C64XX_GPG(6), S3C_GPIO_SFN(3));
...
/* Added by woong */
void s3c6410_setup_sdhci0_cfg_gpio_for_sleep(void)
{
	//janged elisa는 데모 보드와 비교 하면 channel 0와 1이 반대임 
...
EXPORT_SYMBOL(s3c6410_setup_sdhci0_cfg_gpio_for_sleep);

void s3c6410_setup_sdhci1_cfg_gpio_for_sleep(void)
{
	//janged elisa는 데모 보드와 비교 하면 channel 0와 1이 반대임 
	unsigned int gpio;
	unsigned int end;
...
EXPORT_SYMBOL(s3c6410_setup_sdhci1_cfg_gpio_for_sleep);
