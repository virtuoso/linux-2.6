...
//janged add start
static inline int s3c_gpio_do_getcfg(struct s3c_gpio_chip *chip,
				     unsigned int off)
{
	return (chip->config->get_config)(chip, off);
}

static inline int s3c_gpio_do_getpull(struct s3c_gpio_chip *chip,
				      unsigned int off)
{
...
//janged
extern unsigned s3c_gpio_getcfg_s3c24xx(struct s3c_gpio_chip *chip,
				   unsigned int off);
